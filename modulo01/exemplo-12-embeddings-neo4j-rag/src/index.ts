...
try {
    ...
    for (const index in questions) {
        ...
        const result = await ai.anwserQuestion(question!)
        if (result.error) {
        ...
        const fileName = `${CONFIG.output.fileName}_${index}-${Date.now()}.md`
        await writeFile(`${CONFIG.output.answersFolder}/${fileName}`, result.answer!);
    }
